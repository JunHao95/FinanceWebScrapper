...
CC=$(jq -r '.cc | sort | join(",")' config.json)
BCC=$(jq -r '.bcc | sort | join(",")' config.json)
SUMMARY=$(jq -r '.summary' config.json)
SAVEREPORT=$(jq -r '.saveReports' config.json)


# Build command based on config
CMD="python main.py --tickers $TICKERS --sources $SOURCES --format $FORMAT --output-dir $OUTPUT_DIR --email $EMAIL --saveReports $SAVEREPORT --fast-mode" # for testing
