...
file_handler = RotatingFileHandler(
    os.path.join(LOGS_DIR, "webapp.log"), maxBytes=1024 * 1024 * 5, backupCount=5
)
...
logger.info(
    "MongoDB storage disabled for web application - storage only via CLI scripts"
)
...
def run_scrapers_for_ticker(ticker, sources=["all"], alpha_key=None, finhub_key=None):
    ...
    if "all" in sources or "yahoo" in sources:
        scraper_tasks.append(
            ("Yahoo Finance", lambda: YahooFinanceScraper(delay=1).get_data(ticker))
        )

    if "all" in sources or "finviz" in sources:
        scraper_tasks.append(
            ("Finviz", lambda: FinvizScraper(delay=1).get_data(ticker))
        )

    if "all" in sources or "google" in sources:
        scraper_tasks.append(
            ("Google Finance", lambda: GoogleFinanceScraper(delay=1).get_data(ticker))
        )
    ...
    if sentiment_enabled and ("all" in sources or "enhanced_sentiment" in sources):
        ...
        def get_sentiment_data():
            scraper = get_enhanced_sentiment_scraper(
                alpha_vantage_key=alpha_key or "", delay=1
            )
            return scraper._scrape_data(ticker)

        scraper_tasks.append(("Enhanced Sentiment", get_sentiment_data))

    if ("all" in sources or "alphavantage" in sources) and (
        alpha_key or os.environ.get("ALPHA_VANTAGE_API_KEY")
    ):
        scraper_tasks.append(
            (
                "Alpha Vantage",
                lambda: AlphaVantageAPIScraper(api_key=alpha_key, delay=1).get_data(
                    ticker
                ),
            )
        )

    if ("all" in sources or "finhub" in sources) and (
        finhub_key or os.environ.get("FINHUB_API_KEY")
    ):
        scraper_tasks.append(
            (
                "Finhub",
                lambda: FinhubAPIScraper(api_key=finhub_key, delay=1).get_data(ticker),
            )
        )

    if ("all" in sources or "technical" in sources) and (
        alpha_key or os.environ.get("ALPHA_VANTAGE_API_KEY")
    ):

        def get_technical_data():
            tech_indicators = TechnicalIndicators(
                api_key=alpha_key, config=webapp_config
            )
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(run_scraper, name, func) for name, func in scraper_tasks
            ]
...
def fundamental_analysis():
    ...
    try:
        ...
        if not ticker:
            return (
                jsonify({"success": False, "error": "Ticker symbol is required"}),
                400,
            )

        if not stock_data:
            return jsonify({"success": False, "error": "Stock data is required"}), 400
...
def scrape_data():
    ...
    try:
        ...
        portfolio_allocation = data.get(
            "portfolio_allocation"
        )  # Get custom allocation from UI

        # Validate tickers
        if not isinstance(tickers, list) or len(tickers) == 0:
            return jsonify({"success": False, "error": "Invalid tickers format"}), 400
        ...
        if len(tickers) == 1:
            ...
            ticker_data = run_scrapers_for_ticker(
                ticker, sources, alpha_key, finhub_key
            )
            ...
            def process_ticker(ticker):
                ...
                logger.info(f"Processing ticker: {ticker}")
                return ticker, run_scrapers_for_ticker(
                    ticker, sources, alpha_key, finhub_key
                )
            ...
            logger.info(
                f"Using {max_ticker_workers} parallel workers for {len(tickers)} tickers"
            )
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=max_ticker_workers
            ) as executor:
                futures = [
                    executor.submit(process_ticker, ticker) for ticker in tickers
                ]
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ticker, ticker_data = future.result(
                            timeout=90
                        )  # 90 second timeout per ticker
                        all_data[ticker] = ticker_data
                        logger.info(f"Successfully processed ticker: {ticker}")
                    except concurrent.futures.TimeoutError:
                        logger.error(f"Timeout processing ticker")  # noqa: F541
                    except Exception as e:
                        logger.error(f"Error processing ticker: {str(e)}")

        logger.info(
            f"Total tickers processed: {len(all_data)}, Tickers: {list(all_data.keys())}"
        )
        ...
        valid_tickers = [
            ...
            and "error" not in d
            and len(
                [k for k in d.keys() if k not in ["Ticker", "Data Timestamp", "error"]]
            )
            >= 5
        ]
        logger.info(
            f"Valid tickers with data: {len(valid_tickers)} out of {len(all_data)}"
        )
        ...
        logger.info(
            f"Will run analytics for {len(analytics_tickers)} tickers: {analytics_tickers}"
        )
        ...
        if len(analytics_tickers) >= 1 and not skip_analytics:
            try:
                ...
                if portfolio_allocation:
                    logger.info(
                        f"Using custom portfolio allocation from UI: {portfolio_allocation}"
                    )
                ...
                tickers_list = analytics_tickers  # Use all input tickers for analytics (yfinance fetches data independently)  # noqa: E501

                # Correlation Analysis (requires 2+ tickers)
                if len(tickers_list) >= 2:
                    try:
                        logger.info(
                            f"Computing correlation analysis for {len(tickers_list)} tickers..."
                        )
                        correlation_result = analytics.correlation_analysis(
                            tickers_list, days=252
                        )
                        if correlation_result and "error" not in correlation_result:
                            analytics_data["correlation"] = correlation_result
                            logger.info(
                                f"✓ Correlation analysis completed successfully"  # noqa: F541
                            )
                        else:
                            logger.warning(
                                f"Correlation analysis returned error: {correlation_result.get('error', 'Unknown error')}"  # noqa: E501
                            )
                    except Exception as e:
                ...
                for ticker in tickers_list:
                    ...
                    try:
                        if ticker in all_data and all_data[ticker]:
                            logger.info(
                                f"Computing fundamental analysis for {ticker}..."
                            )
                            fundamental_result = analytics.fundamental_analysis(
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Regression analysis failed for {ticker}: {str(e)}"
                        )

                    try:
                        mc_result = analytics.monte_carlo_var_es(
                            [ticker], days=252, simulations=5000
                        )
                ...
                if len(tickers_list) >= 2:
                    try:
                        logger.info(
                            f"Running portfolio-level Monte Carlo for {len(tickers_list)} tickers..."
                        )
                        portfolio_mc_result = analytics.monte_carlo_var_es(
                            tickers_list, days=252, simulations=5000
                        )
                        if portfolio_mc_result and "error" not in portfolio_mc_result:
                            analytics_data["portfolio_monte_carlo"] = (
                                portfolio_mc_result
                            )
                            logger.info(
                                f"✓ Portfolio Monte Carlo completed (includes stress test)"  # noqa: F541
                            )
                        else:
                            logger.warning(
                                f"Portfolio Monte Carlo returned error: {portfolio_mc_result.get('error', 'Unknown')}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Portfolio Monte Carlo analysis failed: {str(e)}"
                        )

                # PCA Analysis (if 3+ tickers)
                if len(tickers_list) >= 3:
                    try:
                        pca_result = analytics.pca_analysis(
                            tickers_list,
                            days=252,
                            n_components=min(3, len(tickers_list)),
                        )
            ...
            logger.info(
                f"Skipping advanced analytics for large portfolio ({len(analytics_tickers)} tickers). Enable for smaller portfolios (≤50 tickers)."  # noqa: E501
            )
            analytics_data["info"] = {
...
def send_email_report():
    ...
    try:
        ...
        if not recipients:
            return (
                jsonify({"success": False, "error": "No recipient email provided"}),
                400,
            )
...
def calculate_greeks():
    ...
    try:
        ...
        return jsonify({"success": True, "greeks": greeks})
...
def convergence_analysis():
    ...
    try:
        ...
        return jsonify({"success": True, "convergence_data": convergence_data})
...
def volatility_surface():
    ...
    try:
        ...
        if "ticker" not in data:
            return (
                jsonify({"success": False, "error": "Missing required field: ticker"}),
                400,
            )
        ...
        return jsonify({"success": True, "surface": surface_data})
...
def atm_term_structure():
    ...
    try:
        ...
        term_structure = builder.get_atm_volatility_term_structure(
            ticker=ticker, risk_free_rate=risk_free_rate, option_type=option_type
        )

        # Convert numpy types for JSON serialization
        term_structure = convert_numpy_types(term_structure)

        return jsonify({"success": True, "term_structure": term_structure})
...
def regime_detection_endpoint():
    ...
    try:
        ...
        if "ticker" in data or "start_date" in data or "end_date" in data:
            ...
            _cache_key = (ticker, start_date, end_date)
            if _cache_key in _regime_cache:
                return jsonify(_regime_cache[_cache_key])
            ...
            if hist.empty:
                return (
                    jsonify({"success": False, "error": f"No data for {ticker}"}),
                    400,
                )
        ...
        if full_probs.ndim == 2 and full_probs.shape[1] == 2:
            ...
            if stressed_prob_value is not None:
                ...
                if abs(last_row[0] - stressed_prob_value) < abs(
                    last_row[1] - stressed_prob_value
                ):
                    stressed_col = 0
                else:
                    stressed_col = 1
        ...
        _regime_cache[_cache_key] = convert_numpy_types(response)
        return jsonify(_regime_cache[_cache_key])
...
def calibrate_heston_stream():
    ...
    def generate():
        try:
            ...
            calibrator = HestonCalibrator()
            for event in calibrator.calibrate_stream(
                ticker, rate, option_type=option_type
            ):
                yield event
        except Exception as e:
            import json
...
def calibrate_bcc_endpoint():
    ...
    try:
        ...
        result = calibrator.calibrate(
            ticker, risk_free_rate=risk_free_rate, option_type=option_type
        )
...
def interest_rate_model_endpoint():
    ...
    """
    try:
        from src.analytics.interest_rate_models import CIRCalibrator  # noqa: F401
        from src.analytics.interest_rate_models import (
            cir_yield_curve,
            calibrate_to_treasuries,
            vasicek_yield_curve,
        )
...
def markov_chain_endpoint():
    ...
    try:
        ...
        P = (
            ...
            else SP_TRANSITION_MATRIX.copy()
        )
    ...
    try:
        ...
        if groq_api_key:
            ...
            reply = (
                ...
                .get("message", {})
                .get("content", "No response content found.")
            )
        ...
        return jsonify(
            {
                "reply": f"Error communicating with LLM. Details: {str(e)}. Response: {error_body}"
            }
        )
    except Exception as e:
...
_ticker_validation_cache: TTLCache = TTLCache(
...
# Note: TTLCache is per-worker (not shared across Gunicorn workers). Each worker has its own cache — expected and safe.  # noqa: E501
...
def get_peers():
    ...
    try:
        ...
        if known_sector and known_sector in _peer_cache:
            ...
            ticker_row = next(
                (r for r in cached_peer_data if r["ticker"] == ticker), None
            )
            if ticker_row:
                percentiles = {
                    "pe": {
                        ...
                        "rank": percentile_rank(
                            cached_peer_data, "pe", ticker_row["pe"]
                        ),
                    },
                    "pb": {
                        "value": ticker_row["pb"],
                        "rank": percentile_rank(
                            cached_peer_data, "pb", ticker_row["pb"]
                        ),
                    },
                    "roe": {
                        "value": ticker_row["roe"],
                        "rank": percentile_rank(
                            cached_peer_data, "roe", ticker_row["roe"]
                        ),
                    },
                    "op_margin": {
                        "value": ticker_row["op_margin"],
                        "rank": percentile_rank(
                            cached_peer_data, "op_margin", ticker_row["op_margin"]
                        ),
                    },
                }
                return jsonify(
        ...
        if len(peer_data) < 2:
            return jsonify(
                {"error": "Peer data unavailable: fewer than 2 comparable companies"}
            )

        # Store in sector-scoped cache and update ticker->sector map
        _peer_cache[sector] = {"data": peer_data, "peers": raw.get("peers", [])}
