    ...
    def _scrape_data(self, ticker):
        ...
        try:
                ...
                for row in rows:
                    ...
                    if len(cols) >= 2:
                        ...
                        # Additional metrics
                            ...
                            "Enterprise Value/EBITDA" in header or "EV/EBITDA" in header
                        ):
                            ...
                            "Return on Investment" in header
                            or "Return on Capital" in header
                        ):
            ...
            try:
                        ...
                        for row in rows:
                            ...
                            if len(cells) >= 2:
                                ...
                                if "current year" in header and len(cells) > 1:
                                    data["EPS Estimate Current Year (Yahoo)"] = cells[
                                        1
                                    ].text.strip()
                                elif "next year" in header and len(cells) > 1:
                                    data["EPS Estimate Next Year (Yahoo)"] = cells[
                                        1
                                    ].text.strip()
                ...
                self.logger.warning(
                    f"Error scraping Yahoo Finance analysis page: {str(e)}"
                )
        ...
        try:
            ...
            if "P/S Ratio (Yahoo)" not in data and info.get(
                "priceToSalesTrailing12Months"
            ):
                data["P/S Ratio (Yahoo)"] = (
                    f"{info.get('priceToSalesTrailing12Months'):.2f}"
                )
            ...
            if "Operating Margin (Yahoo)" not in data and info.get("operatingMargins"):
                data["Operating Margin (Yahoo)"] = (
                    f"{info.get('operatingMargins')*100:.2f}%"
                )
            ...
            try:
                ...
                self.logger.warning(
                    f"Error fetching Total Assets for {ticker}: {str(e)}"
                )
            ...
            self.logger.warning(
                f"Error fetching yfinance API data for {ticker}: {str(e)}"
            )
