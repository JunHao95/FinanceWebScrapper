...
class EnhancedSentimentScraper(BaseScraper):
    ...
    def _scrape_data(self, ticker: str) -> dict:
        ...
        try:
            ...
            formatted_data["Overall Sentiment Score (Enhanced)"] = (
                f"{overall.get('score', 0):.3f}"
            )
            formatted_data["Overall Sentiment Label (Enhanced)"] = overall.get(
                "label", "No Data"
            )
            formatted_data["Sentiment Confidence (Enhanced)"] = (
                f"{overall.get('confidence', 0):.3f}"
            )
            formatted_data["Active Data Sources (Enhanced)"] = overall.get(
            ...
            if not trends.get("error"):
                ...
                formatted_data["Google Trends Interest (Enhanced)"] = (
                    latest_interest
                    if isinstance(latest_interest, (int, float))
                    else latest_interest
                )
                formatted_data["Trends Direction (Enhanced)"] = trends.get(
                    "trend_direction", "No Data"
                )
                formatted_data["Avg Interest (Enhanced)"] = (
                    f"{avg_interest:.1f}"
                    if isinstance(avg_interest, (int, float))
                    else str(avg_interest)
                )
            ...
            if not news.get("error"):
                ...
                formatted_data["News Articles Analyzed (Enhanced)"] = news_overall.get(
                    "total_articles", 0
                )
                formatted_data["News Sentiment Score (Enhanced)"] = (
                    f"{news_overall.get('vader_avg_sentiment', 0):.3f}"
                )
                formatted_data["Positive News Articles (Enhanced)"] = news_overall.get(
                    "positive_articles", 0
                )
                formatted_data["Negative News Articles (Enhanced)"] = news_overall.get(
                    "negative_articles", 0
                )
                finbert_val = news_overall.get("finbert_avg_sentiment")
                formatted_data["FinBERT News Score (Enhanced)"] = (
                    f"{finbert_val:.3f}" if finbert_val is not None else "--"
                )
            ...
            if not reddit.get("error"):
                ...
                formatted_data["Reddit Posts Analyzed (Enhanced)"] = reddit_overall.get(
                    "total_posts", 0
                )
                formatted_data["Reddit Sentiment Score (Enhanced)"] = (
                    f"{reddit_overall.get('avg_sentiment', 0):.3f}"
                )
                formatted_data["Reddit Avg Score (Enhanced)"] = (
                    f"{reddit_overall.get('avg_score', 0):.1f}"
                )
                formatted_data["Reddit Avg Comments (Enhanced)"] = (
                    f"{reddit_overall.get('avg_comments', 0):.1f}"
                )
                formatted_data["Positive Reddit Posts (Enhanced)"] = reddit_overall.get(
                    "positive_posts", 0
                )
            ...
            if not topics.get("error") and "topics" in topics:
                ...
                for i, topic in enumerate(top_topics):
                    ...
                    formatted_data[f"Top Topic {i+1} Keywords (Enhanced)"] = (
                        topic_keywords
                    )
                formatted_data["Document Similarity (Enhanced)"] = (
                    f"{topics.get('avg_document_similarity', 0):.3f}"
                )
            ...
            self.logger.error(
                "Error in enhanced sentiment analysis for %s: %s", ticker, e
            )
