...
from ..utils.exchange_utils import get_exchange_info
...
class GoogleFinanceScraper(BaseScraper):
    ...
    def _scrape_data(self, ticker):
        ...
        """
        exch = get_exchange_info(ticker)
        # For non-US tickers with a known Google exchange code, try that first.
        # For US tickers, try NASDAQ then NYSE then bare URL.
        if not exch["is_us"] and exch["google_exchange"]:
            base = ticker.rsplit(".", 1)[0] if "." in ticker else ticker
            urls = [
                f"https://www.google.com/finance/quote/{base}:{exch['google_exchange']}",
        ...
        for url in urls:
            try:
                    ...
                    for metric in metrics:
                        ...
                        if value_div:
                            ...
                            # Additional metrics
                                ...
                                "roic" in label or "return on invested capital" in label
                            ):
                                data["ROIC (Google)"] = value
                            elif (
                                "ev/ebitda" in label
                                or "enterprise value to ebitda" in label
                            ):
                ...
                self.logger.warning(
                    f"Failed to fetch Google Finance data from {url}: {str(e)}"
                )
                continue
        ...
        if data:
            ...
            self.logger.warning(
                f"Could not find any metrics for {ticker} on Google Finance"
            )
