...
class FinancialAnalytics:
    ...
    @staticmethod
    def compute_pct_increase(
        base: float, new: float, initial_investment: float
    ) -> Optional[float]:
        """
    ...
    def _get_portfolio_weights(self, tickers: List[str]) -> Dict[str, float]:
        ...
        if weights:
            ...
            if missing_tickers:
                ...
                equal_share = (
                    remaining_weight / len(missing_tickers) if missing_tickers else 0
                )
                for ticker in missing_tickers:
                    weights[ticker] = equal_share

            self.logger.info(
                f"Using portfolio weights from config for {len(weights)} tickers"
            )
            return weights

        # Fall back to equal weights
        self.logger.info(
            f"No config weights found, using equal weights for {len(tickers)} tickers"
        )
        return {ticker: 1.0 / len(tickers) for ticker in tickers}

    def get_historical_returns(
        self, tickers: List[str], days: int = 252, return_type: str = "simple"
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if isinstance(prices, pd.DataFrame):
                ...
                if failed_tickers:
                    self.logger.warning(
                        f"Failed to fetch data for tickers: {failed_tickers}"
                    )
                successful_tickers = list(prices.columns)
                if successful_tickers:
                    self.logger.info(
                        f"Successfully fetched data for {len(successful_tickers)} tickers: {successful_tickers}"
                    )
            ...
            self.logger.info(
                f"Successfully fetched {len(returns)} days of returns for {len(returns.columns)} tickers"
            )
            return returns
    ...
    def linear_regression_analysis(
        self, tickers: List[str], benchmark: str = "^GSPC", days: int = 252
    ) -> Dict:
        """
        ...
        try:
            self.logger.info(
                f"Performing linear regression analysis for {len(tickers)} tickers"
            )
            ...
            for ticker in tickers:
                ...
                try:
                    ...
                    correlation = float(
                        np.corrcoef(
                            benchmark_returns.flatten(), ticker_returns.flatten()
                        )[0, 1]
                    )
                    ...
                    results[ticker] = {
                        ...
                        "Interpretation": self._interpret_regression(
    ...
    def pca_analysis(
        self, tickers: List[str], days: int = 252, n_components: Optional[int] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            principal_components = pca.fit_transform(returns_standardized)  # noqa: F841

            # Create results dictionary
            results = {
                "Standardization": {
                    "Mean": {
                        ticker: round(float(mean), 6)
                        for ticker, mean in zip(returns.columns, scaler.mean_)
                    },
                    "Std Dev": {
                        ticker: round(float(std), 6)
            ...
            for i in range(len(pca.explained_variance_ratio_)):
                ...
                results["Interpretation"][pc_name] = self._interpret_pc(
                    loadings[i], returns.columns, pca.explained_variance_ratio_[i]
                )
    ...
    def _interpret_pc(
        self, loadings: np.ndarray, tickers: List[str], variance_explained: float
    ) -> str:
        """
    ...
    def _components_for_variance_threshold(
        self, explained_variance_ratio: np.ndarray, threshold: float
    ) -> int:
        """
    ...
    def monte_carlo_var_es(
        ...
        try:
            self.logger.info(
                f"Running Monte Carlo simulation ({model.upper()}): {simulations} scenarios"
            )
            ...
            if model.lower() == "merton":
                ...
                # GBM component (daily)
                gbm_daily = np.random.normal(
                    drift, portfolio_std, (simulations, forecast_days)
                )
                ...
                simulated_returns = np.random.normal(
                    portfolio_mean, portfolio_std, (simulations, forecast_days)
                )
            ...
            )  # 75th percentile (better than 75% of outcomes)
            bottom_10_percentile = float(
                np.percentile(final_values, 10)
            )  # 10th percentile (worse than 90% of outcomes)
            ...
            results = {
                ...
                "Portfolio Statistics": {
                    ...
                    "Annualized Volatility": round(
                        float(portfolio_std * np.sqrt(252) * 100), 2
                    ),
                },
                "Scenario Analysis": {
            ...
            if len(tickers) >= 2:
                try:
                    self.logger.info(
                        f"=== STRESS TEST START === Running for {len(tickers)} tickers"
                    )
                    ...
                    if stress_results and "error" not in stress_results:
                        ...
                        self.logger.info(
                            f"=== STRESS TEST COMPLETE === Added to results with keys: {list(stress_results.keys())}"
                        )
                    else:
                        self.logger.warning(
                            f"Stress test returned error: {stress_results.get('error', 'Unknown')}"
                        )
                except Exception as stress_error:
                    self.logger.warning(
                        f"Stress test failed with exception: {str(stress_error)}"
                    )
            else:
                self.logger.info(
                    f"Skipping stress test - only {len(tickers)} ticker(s), need 2+ for multi-asset stress test"
                )
    ...
    def stress_test_var(
        ...
        Args:
            ...
            degrees_of_freedom (int): DoF for Student-t (lower = fatter tails, default 3), lower df captures more extreme events  # noqa: E501
        ...
        try:
            self.logger.info(
                f"Running stress test VaR simulation: {simulations} scenarios"
            )
            ...
            if len(tickers) == 1:
                self.logger.warning(
                    "Stress test requires multiple assets. Single asset provided."
                )
                ...
                return {
                    ...
                    "Stress Impact": {
                        ...
                        "VaR Increase %": self.compute_pct_increase(
                            base_var, stress_var, initial_investment
                        ),
                    },
                    "Parameters": {
                        "Assets": len(tickers),
            ...
            mean_returns = returns.mean().values  # noqa: F841
            ...
            for i in range(len(tickers)):
                for j in range(len(tickers)):
                    if i == j:
                        ...
                        cov_stress[i, j] = (
                            rho_stress * vol_stress_vec[i] * vol_stress_vec[j]
                        )
            ...
            results = {
                ...
                "Stress Case": {
                    ...
                    "Distribution": (
                        "Student-t (Fat Tails)" if use_fat_tails else "Normal"
                    ),
                    "Liquidity Haircut": f"{liquidity_haircut * 100}%",
                    "Interpretation": (
                        f"Under EXTREME crisis conditions (fat tails + volatility spike + correlation breakdown + liquidity crisis), "  # noqa: E501
                        f"with {confidence_level*100}% confidence, maximum loss is ${var_stress:,.2f} ({var_stress_pct:.2f}%)"  # noqa: E501
                    ),
                },
                "Stress Impact": {
                    "VaR Increase": round(float(var_stress - var_base), 2),
                    "VaR Increase %": self.compute_pct_increase(
                        var_base, var_stress, initial_investment
                    ),
                    "VaR 99% Increase": round(float(var_99_stress - var_base), 2),
                    "ES Increase": round(float(es_stress - es_base), 2),
                    "ES Increase %": self.compute_pct_increase(
                        es_base, es_stress, initial_investment
                    ),
                    "Prob Loss Increase": round(
                        float(prob_loss_stress - prob_loss_base), 2
                    ),
                    "Interpretation": (
                        f"LEPTOKURTIC STRESS: Crisis scenario increases VaR by ${var_stress - var_base:,.2f} "
                        f"({self.compute_pct_increase(var_base, var_stress, initial_investment) or 0.0:.1f}% of portfolio). "  # noqa: E501
                        f"Fat-tailed distribution captures extreme 'Black Swan' events. "
                        f"Extreme stress (99% VaR) shows ${var_99_stress:,.2f} loss - "
                        f"{self.compute_pct_increase(var_base, var_99_stress, initial_investment) or 0.0:.1f}% of portfolio from base case"  # noqa: E501
                    ),
                },
                "Parameters": {
                    "Assets": len(tickers),
                    ...
                    "Degrees of Freedom": (
                        degrees_of_freedom if use_fat_tails else "N/A"
                    ),
                    "Liquidity Haircut": f"{liquidity_haircut * 100}%",
                    "Downside Asymmetry": (
                        "20% amplification" if use_fat_tails else "None"
                    ),
                },
                "Portfolio Composition": {
                    ticker: round(weight * 100, 2)
    ...
    def correlation_analysis(
        self, tickers: List[str], days: int = 252, method: str = "pearson"
    ) -> Dict:
        """
        ...
        try:
            self.logger.info(
                f"Performing correlation analysis for {len(tickers)} tickers"
            )
            ...
            self.logger.info(
                f"Returns shape after fetch: {returns.shape if not returns.empty else 'EMPTY'}"
            )
            self.logger.info(
                f"Columns in returns: {list(returns.columns) if not returns.empty else 'NONE'}"
            )

            if returns.empty:
                self.logger.warning(
                    "No returns data fetched - all tickers may have failed to download"
                )
                return {"error": "Unable to fetch historical data for any tickers"}

            if len(returns.columns) < 2:
                self.logger.warning(
                    f"Only {len(returns.columns)} ticker(s) with data, need at least 2"
                )
            ...
            self.logger.info(
                f"After dropping NaN columns: {len(returns.columns)} tickers remaining"
            )

            if len(returns.columns) < 2:
                self.logger.warning(
                    f"After filtering NaN, only {len(returns.columns)} ticker(s) remain"
                )
                return {
                    "error": "Insufficient data for correlation analysis after filtering"
                }
            ...
            results = {
                ...
                "Interpretation": self._interpret_correlation_matrix(
                    avg_correlation, len(high_corr_pairs), len(negative_corr_pairs)
                ),
            }
    ...
    def _interpret_correlation_matrix(
        self, avg_correlation: float, high_corr_count: int, negative_corr_count: int
    ) -> str:
        """
    ...
    def comprehensive_analysis(
        ...
        results = {
            ...
            "2. Correlation Analysis": self.correlation_analysis(tickers, days),
            "3. PCA Analysis": self.pca_analysis(tickers, days),
    ...
    def fundamental_analysis(self, stock_data: Dict, ticker: str) -> Dict:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Available data keys for {ticker}: {list(stock_data.keys())[:20]}"
            )
            ...
            financial_score = self._analyze_financial_health(
                stock_data, strengths, concerns
            )
    ...
    def _analyze_valuation(
        self, data: Dict, strengths: List[str], concerns: List[str]
    ) -> Optional[float]:
        ...
        try:
            ...
            if pe:
                ...
                if pe < 15:  # undervalued, a good buy
                    ...
                    score -= 1.5
                    concerns.append(
                        f"High P/E ratio of {pe:.2f} suggests overvaluation"
                    )

            # P/B Ratio
            pb = self._extract_metric(
                data, ["P/B Ratio", "Price to Book", "Price/Book"]
            )
            if pb:
                metrics_found += 1
            ...
            # P/S Ratio
            ps = self._extract_metric(
                data, ["P/S Ratio", "Price to Sales", "Price/Sales", "PS Ratio"]
            )
            if ps:
                metrics_found += 1
                if ps < 2:  # cheap
                    ...
                    score -= 1.0
                    concerns.append(
                        f"High P/S ratio of {ps:.2f} suggests overvaluation"
                    )
            ...
            if peg:
                ...
                if peg < 1:  # very undervalued, high growth potential
                    ...
                    score += 0.5
                    strengths.append(
                        f"PEG ratio of {peg:.2f} suggests normal valuation"
                    )
                elif peg > 2:
                    score -= 0.5
    ...
    def _analyze_profitability(
        ...
        try:
            ...
            roe = self._extract_metric(
                data, ["ROE", "Return on Equity", "Return On Equity", "ROE %"]
            )
            if roe:
                metrics_found += 1
                if abs(roe) > 500:
                    ...
                    concerns.append(
                        f"ROE of {roe:.2f}% likely reflects negative equity (not a profitability signal)"
                    )
                elif roe > 20:
                    score += 2.0
            ...
            # Profit Margin
            profit_margin = self._extract_metric(
                data,
                [
                    "Profit Margin",
                    "Net Margin",
        ...
        try:
            ...
            if current_ratio:
                ...
                if current_ratio > 2.0:
                    score += 1.0
                    strengths.append(
                        f"Strong liquidity with current ratio of {current_ratio:.2f}"
                    )
                elif current_ratio < 1.0:
                    score -= 1.5
                    concerns.append(
                        f"Liquidity concerns with current ratio of {current_ratio:.2f}"
                    )
            ...
            # Operating Cash Flow
            ocf = self._extract_metric(
                data, ["Operating Cash Flow", "OCF", "Cash from Operations"]
            )
            if ocf:
                metrics_found += 1
                if ocf > 0:
            ...
            # EBITDA: profitability measure that looks at a company's earnings before interest, taxes, depreciation, and amortization  # noqa: E501
    ...
    def _analyze_growth(
        ...
        try:
            ...
            # Earnings Growth
            earnings_growth = self._extract_metric(
                data, ["Earnings Growth", "EPS Growth"]
            )
            if earnings_growth:
                metrics_found += 1
                if earnings_growth > 15:
                    score += 1.5
                    strengths.append(
                        f"Strong earnings growth of {earnings_growth:.2f}%"
                    )
                elif earnings_growth > 0:
                    score += 0.25
                    strengths.append(
                        f"Positive earnings growth of {earnings_growth:.2f}%"
                    )
                elif earnings_growth < 0:
                    score -= 1.0
