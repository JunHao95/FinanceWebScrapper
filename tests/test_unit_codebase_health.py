...
def test_percentile_rank():
    ...
    ), "webapp.py must use bisect.bisect_left (BUG-03)"
...
def test_secret_key_guard():
    ...
    # Run from a temp dir so load_dotenv() cannot find the project .env file.
...
def test_procfile_workers():
    ...
    with open(procfile) as fh:
        content = fh.read()
    assert "--workers 2" in content, "Procfile must include --workers 2 (PERF-03)"
...
def test_requirements_pinned():
    ...
    unpinned = [
        ln.strip()
        for ln in lines
        if re.match(r"[A-Za-z].*>=", ln.strip()) and not ln.startswith("#")
    ]
    assert not unpinned, f"Unpinned requirements (must use ==): {unpinned}"
...
def test_precommit_config():
    ...
    assert os.path.exists(
        os.path.join(_ROOT, ".pre-commit-config.yaml")
    ), ".pre-commit-config.yaml must exist (TECH-01)"
    assert os.path.exists(
        os.path.join(_ROOT, ".flake8")
...
def test_regime_cache():
    ...
    with open(_WEBAPP) as fh:
        source = fh.read()
    cached = (
        "lru_cache" in source or "_regime_cache" in source or "regime_cache" in source
    )
    assert cached, "webapp.py must memoize HMM regime detection (PERF-01)"
