...
def test_golden_path_all_tabs_render(page, flask_server):
    ...
    page.route(
        "**/api/trading_indicators**",
        lambda route: route.fulfill(
            status=200, content_type="application/json", body=_MOCK_TRADING_INDICATORS
        ),
    )
    page.route(
        "**/api/footprint**",
        lambda route: route.fulfill(
            status=200, content_type="application/json", body=_MOCK_FOOTPRINT
        ),
    )
    page.route(
        "**/api/regime_detection**",
        lambda route: route.fulfill(
            status=200, content_type="application/json", body=_MOCK_REGIME
        ),
    )
    page.route(
        "**/api/stoch_portfolio_mdp**",
        lambda route: route.fulfill(
            status=200, content_type="application/json", body=_MOCK_PORTFOLIO_MDP
        ),
    )
    ...
    analytics_text = page.evaluate(
        "document.getElementById('analyticsTabContent').innerText"
    )
    assert len(analytics_text.strip()) > 0, "Analytics tab has no visible content"

    # --- Tab 3: Auto Analysis (still exists, verify accessible) ---
    page.click("#autoanalysisTab")
    assert page.locator("#autoanalysisTabContent").is_visible()

    # --- Tab 5: ML Signals (default after analysis run) ---
    page.click("#mlSignalsTab")
    assert page.locator("#mlSignalsTabContent").is_visible()
    ...
    trading_text = page.evaluate(
        "document.getElementById('tradingIndicatorsTabContent').innerText"
    )
    assert (
        len(trading_text.strip()) > 0
    ), "Trading Indicators tab has no visible content"
